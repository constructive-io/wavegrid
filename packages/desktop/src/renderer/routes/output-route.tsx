...
import { OscPanel } from '@/renderer/routes/osc-panel';
import type { BrainStatus, OscTarget } from '@/types/ipc';
...
const KINDS: { id: Kind; label: string; blurb: string }[] = [
  {
    id: 'beyond',
    label: 'BEYOND',
    blurb: 'Pangolin BEYOND over OSC — the usual choice'
  },
  { id: 'fb4', label: 'FB4', blurb: 'Pangolin FB4 over OSC' },
  {
    id: 'routing',
    label: 'Routing file',
    blurb: 'A multi-target routing JSON on disk'
  },
  {
    id: 'none',
    label: 'None',
    blurb: 'Console only — nothing is sent to hardware'
  }
];
 ...
 *
 * Advanced is the OSC debugger, kept here rather than in the sidebar: the
 * question it answers (“why is nothing lighting up?”) is a question about this
 * screen's target, and its panel only mounts once that tab is opened.
 */
export function OutputRoute({
      ...
      <div className="p-4">
        <Empty>
          <EmptyHeader>
            ...
            <EmptyDescription>
              Select a project to set its laser output.
            </EmptyDescription>
          </EmptyHeader>
        </Empty>
      </div>
  ...
  const dirty =
    target != null && JSON.stringify(draft) !== JSON.stringify(target);
          ...
          <div className="flex flex-col gap-3 rounded-lg border px-4 py-3">
            <div className="flex items-center gap-2">
              ...
              <span className="font-medium">
                Laser output for “{activeProject}”
              </span>
                    ...
                    <div className="flex gap-2">
                      {(['row', 'column'] as const).map((order) => (
                        <Button
                          ...
                          size="sm"
                          variant={
                            draft.gridOrder === order ? 'default' : 'outline'
                          }
                          disabled={busy}
            ...
            {draft.kind === 'none' && (
              <p className="text-muted-foreground text-sm">
                The show runs and the UI animates, but nothing is sent to
                hardware.
              </p>
            )}
